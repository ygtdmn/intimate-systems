...
library EssayPage {
    function html(address show, address essayContract, address data) public view returns (string memory) {
        ...
        string memory body = string.concat(
            ...
            "</article>",
            "<footer>",
            _renderUrls(essay.urls()),
            "<address>",
            showAddr,
            "</address>",
            '<p><i>This text was published as part of the contract show <a href="/">',
            showTitle,
            "</a></i></p>",
            "</footer>",
            "</main>"
    ...
    function _renderUrls(string[] memory urls) internal pure returns (string memory _html) {
        if (urls.length == 0) {
            return "";
        }

        for (uint256 i = 0; i < urls.length; i++) {
            _html = string.concat(_html, '<a href="', urls[i], '" target="_blank" rel="noopener">', urls[i], "</a>");
        }
