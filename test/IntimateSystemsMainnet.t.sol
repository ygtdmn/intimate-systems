...
contract IntimateSystemsMainnetTest is Test {
    function testRenderMainnetToFile() public {
        ...
        for (uint256 i = 0; i < sculptures.length; i++) {
            ...
            // Write token-uri files for the fetch endpoint
            string memory tokenUriContent = MediaPage.tokenUri(address(show), i);
            vm.writeFile(string.concat("html_output/token-uri/", vm.toString(i)), tokenUriContent);
        }
    }
}
